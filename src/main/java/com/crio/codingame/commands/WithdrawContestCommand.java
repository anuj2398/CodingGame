...
public class WithdrawContestCommand implements ICommand{
    ...
    public void execute(List<String> tokens) {
        try{
            UserRegistrationDto userDto=userService.withdrawContest(tokens.get(1), tokens.get(2));
            if(!userDto.getUserName().equals(tokens.get(2))){
            throw new UserNotFoundException("Cannot Withdraw Contest. User for given name:"+ tokens.get(2)+" not found!");
            }
