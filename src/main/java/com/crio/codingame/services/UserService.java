...
class compareScore implements Comparator<User>{
    @Override
    public int compare(User u1, User u2){
        return u1.getScore() - u2.getScore();
    }
}
class compareScoreRev implements Comparator<User>{
    @Override
    public int compare(User u1, User u2){
...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        ...
        if(scoreOrder.equals(ScoreOrder.ASC)){
            Collections.sort(userList,new compareScore());
        }
        else{
            Collections.sort(userList,new compareScoreRev());
        }
        return userList;
    }
